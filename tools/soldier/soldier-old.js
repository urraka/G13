...
function mouse(x, y)
{
	var x = (x - CX) * (1.0 / SCALE);
	var y = (y - CY) * (1.0 / SCALE);
...
Soldier.prototype.draw = function(context)
{
	...
	if (this.running)
	{
		...
		context.transform(1, 0, -0.3, 1, 0, 0);
	}

	context.fillRect(-108, -80, 216, 146);
	context.strokeRect(-108, -80, 216, 146);
	context.restore();

	// legs

	context.save();
	context.translate(90, 66);
	context.beginPath();
	context.moveTo(0, 0);
	...
	context.save();
	context.translate(6, -180);
	context.scale(1, 0.85);
	context.beginPath();
	context.arc(0, 0, 174, 0, 2 * Math.PI);
	context.fill();
	context.stroke();
	context.restore();
